...
function update(currentTime) {
  ...
  let angle = Math.max(
    Math.min((velocityY / 400) * (Math.PI / 6), Math.PI / 6),
    -Math.PI / 6,
  );
  context.save();
  ...
  for (let i = 0; i < pipeArray.length; i++) {
    ...
    if (!pipe.passed && bird.x > pipe.x + pipe.width) {
      ...
      pipe.passed = true;
      // update speed when score increases instead of using a blocking loop
      speedIncrease();
    }
...
const speedIncrease = () => {
  // check the highest threshold first so it isn't shadowed by a lower one
  if (score > 50) {
    velocityX = -6;
